...
@Component({
  selector: 'app-installment-rates',
  standalone: true,
  imports: [
    MaterialModule,
    FormsModule,
  ...
  templateUrl: './installment-rates.component.html',
  styleUrl: './installment-rates.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class InstallmentRatesComponent {
    displayedColumns: string[] = ['#', 'name', 'percent', 'created_by', 'action'];
    ...
    return this.http.get<{ data: any[] }>(`${environment.apiUrl}/installmentPercentage`, { headers }).pipe(
    ...
    editInstallment_rates(area: any): void {
      const dialogRef = this.dialog.open(DialogOverviewExampleDialog, {
        height: '62%',
        width: '40%',
  ...
  // delete
    deleteInstallment_rates(element: PeriodicElement): void {
  
      
      const confirmDelete = confirm(`Are you sure you want to delete ${element.name}?`); // Optional confirmation dialog
      if (confirmDelete) {
        const authToken = localStorage.getItem('authToken'); // Retrieve the token from localStorage
      ...
      const headers = {
        Authorization: `Bearer ${authToken}`,
          ...
          this.http.delete<{ data: any[] }>( `${environment.apiUrl}/installmentPercentage/${element.id}`,
    ...
    const headers = {
      Authorization: `Bearer ${authToken}`,
        ...
        this.http.put<{ data: any[] }>(
          `${environment.apiUrl}/installmentPercentage/${this.data.id}`,
        ...
        this.http.post<{ data: any[] }>(
          `${environment.apiUrl}/installmentPercentage`,
          formData, // formData as the request body
          { headers } // headers passed separately as the third argument
        ).subscribe(
          (response) => {
