...
@Component({
  ...
  styleUrl: './transactions-completed.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,

})
export class TransactionsCompletedComponent {
  displayedColumns: string[] = ['#', 'name_ar', 'email', 'whatsapp', 'Communication_method', 'created_by' , 'action'];
  ...
  return this.http.get<{ data: any[] }>(`${environment.apiUrl}/transactions_completed`, { headers }).pipe(
  ...
  deleteElement(element: PeriodicElement): void {
    ...
    const headers = {
      ...
      //  const url ="https://electronkw.net/new_laravel_API/api/regions";
        ...
        this.http.delete<{ data: any[] }>( `${environment.apiUrl}/transactions_completed/${element.id}`,
    ...
    const headers = {
      Authorization: `Bearer ${authToken}`,
        ...
        this.http.put<{ data: any[] }>(
          `${environment.apiUrl}/transactions_completed/${this.data.id}`,
        ...
        this.http.post<{ data: any[] }>(
          `${environment.apiUrl}/transactions_completed`,
          formData, // formData as the request body
          { headers } // headers passed separately as the third argument
        ).subscribe(
          (response) => {
