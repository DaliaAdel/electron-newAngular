...
export class PoliceStationsComponent {

  displayedColumns: string[] = ['#', 'name_ar', 'name_en', 'region_id', 'created_by', 'action'];
  ...
  return this.http.get<{ data: any[] }>(`${environment.apiUrl}/policeSatations`, { headers }).pipe(
  ...
  deleteElement(element: PeriodicElement): void {
    ...
    const headers = {
      ...
      //  const url ="https://electronkw.net/new_laravel_API/api/regions";
        ...
        this.http.delete<{ data: any[] }>( `${environment.apiUrl}/policeSatations/${element.id}`,
    ...
    const headers = {
      Authorization: `Bearer ${authToken}`,
        ...
        this.http.put<{ data: any[] }>(
          `${environment.apiUrl}/policeSatations/${this.data.id}`,
        ...
        this.http.post<{ data: any[] }>(
          `${environment.apiUrl}/policeSatations`,
          formData, // formData as the request body
          { headers } // headers passed separately as the third argument
        ).subscribe(
          (response) => {
